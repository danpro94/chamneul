...
**백엔드는 전 구간 UTC다** (Owner 결정 2026-09-16).

| 계층 | 규칙 |
| --- | --- |
| **DB** | UTC 저장 (`USE_TZ = True`, `TIME_ZONE = "UTC"`) |
| **백엔드 API** | 비즈니스 로직·네트워크 전송 모두 **ISO 8601 UTC**. 예: `"2026-06-22T05:06:00Z"` |
| **클라이언트** | 사용자 기기/브라우저 타임존을 감지해 현지 시각(KST 등)으로 변환 |

* 백엔드는 현지 시각을 만들지 않는다. 서버가 특정 지역 시간대로 직렬화하면 그 서비스는 그 지역에 묶이고, 사용자가 이동하거나 해외 사용자가 생기면 표시가 틀린다. **변환은 사용자가 어디에 있는지 아는 쪽(클라이언트)의 책임이다.**
* 따라서 `TIME_ZONE`을 지역 시간대로 바꾸는 것은 설정 변경이 아니라 **계약 위반**이다.
* *(2026-09-16 정정: 종전 문서는 "응답은 KST(`+09:00`)로 직렬화"였으나 구현은 처음부터 UTC였다 — 코드가 옳고 문서가 틀렸다.)*
...
| **404** | **전적으로 사적인 자원** — id의 존재 자체를 알려줄 이유가 없다 | #18(내 고민), #39·#40·#41(알림), **#21(내 배정 큐, 2026-09-16 편입)** |
| **403** | 자원은 공유되나 **권한 계층이 미달** | #22·#23·#32·#36·#42·#43(ADMIN 전용), #20·#21의 `active_role≠ADVISOR` |

**이 규칙에 예외는 없다.** 마지막 예외였던 #21이 2026-09-16에 404로 편입됐다.
...
| 접근 제어 조건 | 로그인 사용자, 진행 중 신청 없음. **`APPROVED` 신청은 신청자가 ADVISOR 역할을 실제로 보유한 동안에만 중복으로 판정**한다(2026-09-16 정정) — #43으로 역할이 회수된 사용자는 재신청할 수 있다. Phase 2에는 사용자측 신청 철회 API가 없으므로, 그렇지 않으면 복구 경로가 없다. |
...
| Status | 200 / 401 / 403(`active_role≠ADVISOR`) / 404 / 500 |
| 접근 제어 조건 | 해당 concern에 active 배정이 있는 advisor 본인만. **배정되지 않은 고민은 403이 아니라 404**다(2026-09-16 정정) — 미배정 조언가는 concern id를 알 방법이 없으므로 403은 "그런 고민이 존재한다"를 알려준다(§1.8 404 규칙). 배정이 해제된 경우, 소프트 삭제된 고민도 동일하게 404. `active_role≠ADVISOR`는 권한 계층 미달이라 **403 유지**(§1.8 3행). `requester_display_name`은 서버 파생(C-6/C-7, D-2): `display_alias` 있으면 그것 / 없고 익명이면 "익명의 요청자" / 없고 비익명이면 계정 `nickname`. email·user_id는 미노출. |
| Side Effect | 없음. |
| MVP 여부 | ✓ |
...
| Permission | User |
| Description | 내가 작성한 고민들에 달린 **APPROVED** advice 목록 (CLAUDE.md §6.2). 소프트 삭제한 고민의 조언은 제외된다(Owner 결정 2026-09-13 — #19로 고민을 철회하면 그에 달린 조언도 작성자 화면에서 함께 사라진다). |
| Request 주요 필드 | Query: `keyword?`(concern_summary 대상), `from_date?`, `to_date?`, `page?`, `size?` |
...
| Status | 200 / 400(쿼리 형식 오류) / 401 / 500 |
| 접근 제어 조건 | 본인 고민에 달린 APPROVED advice만. 고민이 소프트 삭제되면 제외. |
| Side Effect | 없음. |
| MVP 여부 | ✓ |
...
| 접근 제어 조건 | 작성자 본인 + `active_role=ADVISOR` + 상태 ∈ {PENDING, REVIEWING}. |
| Side Effect | Advice 본문 갱신. **본문이 실제로 바뀐 경우에만** `version` 증가 + AdviceHistory 행 추가(ADR-007). `submit`만 토글하면 플래그만 갱신된다. |
| MVP 여부 | ✓ |
...
| 접근 제어 조건 | 작성자 본인 + `active_role=ADVISOR` + 상태 ∈ {PENDING, REVIEWING, **REJECTED**}. REJECTED 삭제를 허용하는 이유: 반려 알림이 사유와 함께 해당 조언을 가리키는데 수정·재작성이 모두 막혀 있으면 조언가에게 후속 경로가 없다. 삭제하면 (concern, advisor) 슬롯이 풀려 새로 작성할 수 있다(Owner 결정 2026-09-13). APPROVED 후 삭제는 ADMIN 별도 흐름(Phase 2 v1에서는 미제공, 후순위로 Django Admin 처리). |
...
| Status | 200 / 400(쿼리 형식 오류) / 401 / 403 / 500 |
...
| Request 주요 필드 | Query: `status?` (default `PENDING`, enum 외 값은 400), `page?`, `size?` |
| Response 주요 필드 | `items[]`: { `advice_id`, `concern_id`, `advisor_user_id`, `status`, `version`, `created_at`, `updated_at` }, `page_info` |
| Status | 200 / 400 / 401 / 403 / 500 |
| 접근 제어 조건 | ADMIN. 소프트 삭제된 고민(`deleted_at IS NOT NULL`)에 달린 조언은 제외된다 — #33이 409로 거부하므로 큐에 남기면 리뷰 불가능한 행만 쌓인다(Owner 결정 2026-09-13). |
| Side Effect | 없음. |
| MVP 여부 | ✓ |
...
| Status | 200 / 401 / 403 / 404 / 409(허용되지 않는 전이 · 미제출 초안 · **소프트 삭제된 고민**) / 412(`expected_version` ≠ 현재 version — 리뷰 중 조언가가 수정함, 재조회 유도) / 422 / 500 |
...
| 접근 제어 조건 | 해당 advice가 연결된 concern의 작성자만. advice.status=APPROVED 필수. 고민이 소프트 삭제된 경우 403(Owner 결정 2026-09-13 — #26/#27과 동일 규칙). |
