...
## 7. Owner 결정 — **4건 전부 권고안대로 확정 (2026-09-11)**

api.md를 코드로 옮기는 과정에서 **명세가 답하지 않는** 지점 4건이 나왔다. 전부 실제 동작이 갈리는 지점이라 추측으로 넘기지 않고 Owner 확인을 받았으며, **네 건 모두 아래 "권고" 열대로 확정**되었다. 구현은 이 확정안을 따르고, api.md 문구 반영은 TASK-007에서 일괄 처리한다.
