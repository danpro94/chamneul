...
> Status: **Approved (2026-09-15)** — §7 결정 3건 모두 Owner 승인 완료. TASK-001 착수.
...
## 7. Owner 결정 — 3건 전부 승인 (2026-09-15)

**결정 1 → (a) `actor` 미노출.** **결정 2 → (a) 404.** **결정 3 → (a) 배정 그대로 둔다.**

결정 2에 대한 Owner 근거(원문 보존): *"애초에 이 앱 서비스는 개인화 앱이므로 남의 알림을 열 수 없어야 함. 알림은 나에게만."* — 404가 단순히 응답 코드 선택이 아니라 **알림 자원의 성격 규정**임을 확정한다. 따라서 구현은 "객체를 꺼낸 뒤 소유자를 비교"하는 형태를 쓰지 않고, **쿼리셋 자체를 `recipient=user`로 좁혀** 타인의 알림이 코드 경로에 아예 들어오지 못하게 한다(plan §2).

세 결정 모두 CLAUDE.md 조항과 충돌하지 않는다 — ADR 불필요(§16, ADR-007 교훈 적용 확인).
